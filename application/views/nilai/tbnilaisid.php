    ...
    <div class="col-sm-4 col-sm-offset-4">
      <div class="card">
        ...
        <div class="card-header bg-primary">
          <b>Tambah Nilai sidang</b>
        </div>
        <div class="card-body">   
    
     <form action="<?=base_url('nilai/tbsidang') ?>" method="post">

         <div class="form-group">
                        
                        <select id="id_sidang" name="id_nilai" class="form-control">                            
                            <option value=""> -- Pilih Nama Mahasiswa -- </option>
                         
                           
                            <?php $no = 0; foreach ($sidang as $row): ?>
                                <option value="<?php echo $row->id_sidang; ?>"><?php echo $row->id_sidang; ?> <?php echo $row->npm; ?> </option>                            
                            <?php endforeach; ?>
                    ...
                    <div class="form-group">
                        <label for="pb2" class="form-control-label">NIP Pembimbing 2 :</label>
                        <input id="jenis_seminar" name="jenis_seminar" class="form-control" value="sidang" readonly/>
                    </div>  
                     <div class="form-group">
 ...
 #bagas {
  ...
  <script type="text/javascript">    
    $(document).ready(function(){        
        $('#id_sidang').on('change',function(){            
        var id_sidang=$(this).val();
            $.ajax({
                type : "POST",
                url  : "<?php echo base_url('nilai/getuk')?>",
                dataType : "JSON",
                data : {id_sidang: id_sidang},
                cache:false,
                success: function(data){
                    $.each(data,function(id_sidang,npm,pg1,pg2,$id_bimbing){
  $('[name="id_sidang"]').val(data.id_sidang);
